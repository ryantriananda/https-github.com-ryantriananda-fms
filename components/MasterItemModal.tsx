...
export const MasterItemModal: React.FC<Props> = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center backdrop-blur-[2px] p-4 animate-in fade-in duration-200">
      <div className="bg-[#F8F9FA] w-full max-w-6xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col transform transition-all scale-100 h-[90vh]">
        {/* Header */}
        <div className="px-10 py-8 bg-white border-b border-gray-100 flex items-center justify-between shrink-0">
          <div>
            ...
            <div className="flex items-center gap-4 mt-2">
               <span className="text-[10px] font-bold text-gray-400 tracking-[0.2em] uppercase">Catalog Management</span>
               {form.itemCode && <span className="text-[10px] font-mono font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded">#{form.itemCode}</span>}
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-10 custom-scrollbar bg-[#FBFBFB]">
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Kategori</label>
                    <div className="relative">
                      <select 
                        className="w-full bg-[#F8F9FA] border-none rounded-2xl px-5 py-4 text-[12px] font-black text-black outline-none appearance-none cursor-pointer shadow-sm uppercase focus:ring-2 focus:ring-black/5"
                        value={form.category}
                      ...
                      <Layers size={16} className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                    </div>
                  </div>
                  
                  {/* 2. Item Name Second */}
                  <div className="md:col-span-2">
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Nama Barang</label>
                    <input 
                      type="text" 
                      className="w-full bg-[#F8F9FA] border-none rounded-2xl px-5 py-4 text-[12px] font-black text-black outline-none focus:ring-2 focus:ring-black/5 shadow-sm placeholder:text-gray-300"
                      value={form.itemName || ''}
                  ...
                  <div>
                    ...
                    <div className="relative group">
                      <input 
                        ...
                        className={`w-full border-none rounded-2xl px-5 py-4 text-[12px] font-mono font-bold outline-none shadow-sm ${(!form.category || isView) ? 'bg-gray-50 text-gray-400 cursor-not-allowed' : 'bg-[#F8F9FA] text-black focus:ring-2 focus:ring-black/5'}`}
                        value={form.itemCode || ''}
                        onChange={(e) => setForm({...form, itemCode: e.target.value})}
                        placeholder={form.category ? "CON: TP-001" : "Select Category..."}
                      />
                      {!form.category && !isView && <AlertCircle size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-300" />}
                    </div>
                  </div>

                  {/* 4. UOM */}
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Satuan (UOM)</label>
                    <div className="relative">
                      <select 
                        className="w-full bg-[#F8F9FA] border-none rounded-2xl px-5 py-4 text-[12px] font-black text-black outline-none appearance-none cursor-pointer shadow-sm uppercase focus:ring-2 focus:ring-black/5"
                        value={form.uom}
                      ...
                      <Tag size={16} className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                    </div>
                  </div>

                  {/* 5. Harga Beli */}
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Harga Beli</label>
                    <div className="relative">
                      <span className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 font-black text-xs">Rp</span>
                      <input 
                        type="text" 
                        className="w-full bg-[#F8F9FA] border-none rounded-2xl pl-12 pr-5 py-4 text-[12px] font-black text-black outline-none focus:ring-2 focus:ring-black/5 shadow-sm"
                  ...
                  <div className="md:col-span-2 mt-2">
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 ml-1">Foto Barang / Attachment</label>
                    <input 
                      type="file" 
                      ref={fileInputRef} 
                      className="hidden" 
                    ...
                    <div 
                      ...
                      className={`relative border-2 border-dashed rounded-[1.5rem] h-48 flex flex-col items-center justify-center transition-all overflow-hidden bg-white
                        ${form.imageUrl ? 'border-gray-200' : 'border-gray-200 hover:border-black hover:bg-gray-50/50'}
                    ...
                    >
                      {form.imageUrl ? (
                        <div className="relative w-full h-full group flex items-center justify-center">
                            <img src={form.imageUrl} alt="Preview" className="h-full object-contain p-2" />
                            {!isView && (
                                <button 
                                    onClick={removeImage}
                        ...
                        <div className="flex flex-col items-center p-4">
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm border border-gray-100 transition-all mb-3 bg-white ${isDragging ? 'scale-110' : ''}`}>
              ...
              <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm space-y-6 h-fit">
                <SectionHeader icon={Hash} title="Stock Settings" />
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Min Stock</label>
                      <input 
                        type="number" 
                        className="w-full bg-[#F8F9FA] border-none rounded-2xl px-5 py-4 text-[12px] font-black text-center text-black outline-none focus:ring-2 focus:ring-black/5"
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Max Stock</label>
                      <input 
                        type="number" 
                        className="w-full bg-[#F8F9FA] border-none rounded-2xl px-5 py-4 text-[12px] font-black text-center text-black outline-none focus:ring-2 focus:ring-black/5"
                  ...
                  <div className="p-5 bg-gray-50 rounded-[1.5rem] border border-gray-100 space-y-4">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="text-[12px] font-black text-black">{form.averagePrice || 'Rp 0'}</span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-tight">Last Purchase</span>
                        <span className="text-[12px] font-black text-blue-600">{form.lastPurchasePrice || 'Rp 0'}</span>
                    </div>
                  </div>

                  <div className="flex items-start gap-3 p-5 bg-orange-50 rounded-2xl border border-orange-100">
                      <AlertCircle size={16} className="text-orange-500 shrink-0 mt-0.5" />
                      <p className="text-[10px] font-bold text-orange-700 uppercase leading-relaxed tracking-wide">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-black text-white rounded-lg text-[9px] font-black uppercase tracking-widest mb-4">
              ...
              <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-[#FAFAFA] border-b border-gray-100">
                        <th className="p-5 text-[10px] font-black text-gray-400 uppercase tracking-widest pl-8">PO Number</th>
                        <th className="p-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Tanggal</th>
                        <th className="p-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Vendor</th>
                        <th className="p-5 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Qty</th>
                        <th className="p-5 text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">Harga Satuan</th>
                        <th className="p-5 text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">Total</th>
                        <th className="p-5 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center pr-8">Attachment</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {MOCK_PURCHASE_HISTORY.map((hist) => (
                        <tr key={hist.id} className="hover:bg-gray-50/50 transition-colors">
                          <td className="p-5 font-mono font-bold text-[11px] text-blue-600 pl-8">{hist.id}</td>
                          <td className="p-5 font-bold text-gray-600 text-[11px]">{hist.date}</td>
                          <td className="p-5 font-black text-black text-[11px] uppercase">{hist.vendorName}</td>
                          <td className="p-5 text-center font-black text-black text-[11px]">{hist.qty}</td>
                          <td className="p-5 text-right font-mono font-bold text-[11px] text-gray-500">{hist.unitPrice}</td>
                          <td className="p-5 text-right font-mono font-black text-black text-[11px]">{hist.totalPrice}</td>
                          <td className="p-5 text-center pr-8">
                            {hist.attachmentUrl ? (
                              <img src={hist.attachmentUrl} className="w-8 h-8 rounded-lg object-cover mx-auto border border-gray-100 shadow-sm" />
                            ) : (
                ...
                {MOCK_PURCHASE_HISTORY.length === 0 && (
                  <div className="p-16 text-center bg-white">
                    ...
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Belum ada riwayat pembelian tercatat</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="px-10 py-8 bg-white border-t border-gray-100 flex justify-end gap-4 shrink-0">
          ...
          {!isView && (
            ...
            >
                <Save size={16} strokeWidth={2.5} /> {initialData ? 'Simpan Perubahan' : 'Simpan Barang'}
            </button>
          )}
        </div>
      </div>
